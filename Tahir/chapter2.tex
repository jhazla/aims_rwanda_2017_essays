...
In order to prove what we get is maximum winning  probability we will consider isomorphism between  the first group   which is $G=(\{0,1\},+ \pmod{2})$, and  $ G_1=(\{-1,1\},*)$. In the latter our unitary operators  become hermitian with eigenvalues $\{-1,1\}$ the property need to proof there is  no existence wining probability greater than this for any choices of Operators Tsirelson’s  upper bound inequality~\citep{Cirel'son1980}using  norm of operators  and along with other inequalities .%\footnote{reference3}

First of all, let's drive the probability from the expectation value. Since we are no longer dealing with rotation operators, we can use any Hermitian  operator $H$
...
%$A$ and $B$  win when their output is same, whenever, the only situation were win when their out put is different is $x \text{ and} y$ both are equal to $1$.
...
Returning back to the proof of  Tsirelson’s inequality the right hand side of equation \ref{TE}  can be substituted by
