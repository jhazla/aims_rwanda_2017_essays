...
Statistically, for given a sample space, a probability distribution assigns as a non negative number  to each points of the sample space in such a way that these numbers sum to $1$.
$$\sum_i \Pr(i)=1$$
On the other hand, non deterministic  of quantum mechanics a plays  central for prediction of measurement outcomes, in fact the definition of  probability and means or average in quantum mechanics depends on whether the observable has a discrete or continuous set of eigenvalues, in our case we only dealing with discrete eigenvalues operators, we will introduce the operational definition for this purpose.
If there is observable $A$ for a system with a discrete set of values $\{a_1 , a_2 ,\dots \}$, then this observable is represented by a Hermitian operator $\hat{A}$ , so that has these discrete values are it's eigenvalues, and associated eigenstates $\{\ket{a_n}, n = 1, \dots \}$ satisfying the eigenvalue equation.
\begin{equation}
\hat{A} \ket{a_n}=a_n \ket{a_n}
\end{equation}
These eigenstates form a complete, orthonormal set so that any state ,$\ket{\Psi}$,of the system can be 
\begin{equation}
\ket{\Psi}=\sum_n \ket{a_n} \bra{a_n} \ket{\Psi}
\end{equation}
Where $\bra{\Psi}\ket{\Psi}=1$, and ,$\left|\bra{a_n}\ket{\Psi}\right|^2$ is probability of obtaining $a_n$ by measuring $A$.

Now, suppose we prepare  $N$ identical copies of this system so that all in exactly the same fashion of the  original system, and by  performing  measurement  of $\hat{A}$  for each these copies the result will be completely random,this gives the frequency of probability in quantum    which an outcome will  be occur in the limit of a very large number of repetitions of an experiment.

Using repetition of  measurement  on identical copies of system, we can drive  an expression for  average or mean value of all these results. If the measurement $A$ is repeated $N$ times, the average will be.
 \begin{equation}
 \left\langle A \right\rangle=\sum_n \frac{N_n}{N}  a_n
 \end{equation}
 Where $a_n$  is eigenvalue associated with eigenvector $\ket{a_n}$, and $\frac{N_n}{N}=\left|\bra{a_n}\ket{\Psi}\right|^2$ is the probability of measuring the value $a_n$ from the observable $A$.Moreover, if we have composed state of two operators $A\text{ and} B$ then the eigenvalues of these operators  when both together are all possible the product of the individual eigenvalues , thus, associated  eigenvectors are  tensor product of the vector .
...
\begin{align*}
E[A,B \text{ out put same}]-\Pr[A,B \text{ out put diff} ]\\
E[A,B \text{ out put diff} ]-\Pr[A,B \text{ out put same}  ]
\end{align*}
