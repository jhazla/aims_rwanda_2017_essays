...
Thereby, the density version of Hales-Jewett theorem states that for any positive number $k$ and real
\Jnote{positive}
...
\item The density version of the Hales-Jewett theorem implies   $\lim\limits_{n\longrightarrow \infty} \Delta_{k,n}=0$. We assume that the density version of the Hales-Jewett theorem is true, that is $\forall k, \delta, \exists DHJ(k,\delta)  \in \mathbb{N} / \forall n\geq DHJ(k,\delta)$ and $\forall S \subseteq [k]^n, |S| \geq \delta k^n$, $S$ contains a combinatorial line. This means that  there is a
  \Jnote{s/there is a/for every}
  subset $W \subseteq [k]^n$
  which does not contain a combinatorial line
  \Jnote{for $n$ big enough}
  with density
  \Jnote{its size must be}
...
\begin{proof}
  ...
  \Jnote{Since they are chosen uniformly, do not mention $\mu$.
    Also say in the beginning of this chapter sth like
    ``In this chapter we will assume that all our games sample questions
...
\Jnote{The proofs of 4.2.4 and 4.2.5 are still too close. Ideally, you should
  delete those proofs and write them again yourself, without looking at
  my paper! If you cannot do that, consider writing just the propositions
and refer to the proofs in the paper.}
