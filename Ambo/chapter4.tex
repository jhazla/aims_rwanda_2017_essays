...
Thereby, the density version of Hales-Jewett theorem states that for any positive number $k$ and positive real number $\delta$,  there exists a large enough number $n$ (depending on $k$ and $\delta$) such that  any subset of  $[k]^n $ with density $\delta$ contains a combinatorial line.
...
\begin{proof}
 ...
 % \Jnote{Since they are chosen uniformly, do not mention $\mu$.    Also say in the beginning of this chapter sth like  ``In this chapter we will assume that all our games sample questions  from a uniform distribution.''}
...
%\Jnote{Do not cite with et al., here and elsewhere.}
...
From the definition of the game $G_S$ we can deduce the following propositions given and proved by \cite*{hkazla2016forbidden}. So, the proofs of the two  propositions are adapted from this latter paper.  
...
We have omitted the proof of this proposition. This proof can be found in \cite*{hkazla2016forbidden}.
%\begin{proof}
%We assume that the game $G_S$ is trivial and we show that the set $S$ has a combinatorial line. As the game $G$ is trivial, there is a $k$-tuple of strategies for which the provers  always win.  Let $f^1, \ldots, f^k$ be this $k$-tuple of  strategies. Here, there are two kinds of questions: $q\in \{0,1\}.$ The form of the answer for the game $G_S$ is defined previously as a couple $(T^e, z^e)$ where $e\in [k].$ Thus, according to the strategy $f^e$ the answer to the question $q$ is: $f^e(q)=(T_q^e, z^e_q).$ Note that among all the $k$ provers, there is a special prover called $t$ for which the question $q=1.$
%
%The strategy we have defined makes the game $G_S$ trivial, that is all the four conditions have been accepted by the verifier. This implies:
%\begin{itemize}
%\item Under the first condition that the sets $T^e_q$ where the special prover has the question $q=1$ and other provers have the question $q=0$ form a partition of the set $[n].$ 
%
%Namely, $[n]=T_0^1 \cup \ldots \cup T_0^{t-1}\cup T_1^t  \cup T_0^{t+1} \ldots \cup T_0^k.$
%\item  Under the second condition, all $z^e_q$ are equal. That is, for $z_0^e =z_1^t=z$ where $z_0^t$ for $t \in [k]$ is the second component of the answer for the special prover and $z_0^e$ is for other provers. 
%\item As the sets $T_0^e$ form a partition, that is $T_0^e \cap T_0^{e'} =\emptyset $   for $e \neq e'$  and $T_0^e \cap T_1^{t}= \emptyset$ for $t \neq e.$ Hence, under the third condition, $z \in T_1^t$  and $z \notin T_0^1 \cup \ldots \cup T_0^{t-1} \cup T_0^{t+1} \ldots \cup T_0^k$.
%\item Under the fourth condition the string $\bar{s}= (a_1,\ldots, a_n)$ belongs to $S$ where $a_j=e$ if and only if $j \in T_q^e.$  Thus, let us show that $\bar{s}$ has a combinatorial pattern.
%As the set $[n]$ is partitioned to $k$ sets $T_q^e$ where one set is for the special prover $t$, then we expand the element $a_j$ as:
%\begin{itemize}
%\item If $j \in T_1^t$, then $a_j=x$. That is, if the coordinate $j$ belongs to the set of the special prover $t$, then the element $a_j$ equals to the wildcard $x.$
%\item If $j \in T_0^e$, then $a_j=e$ for $e \in [k].$ 
%\end{itemize}
%Thus, the string $\bar{s}= (a_1,\ldots, a_n)$ forms a combinatorial pattern. 
%\end{itemize}
%Hence, $w(x)=\bar{s}=(a_1,\ldots, a_n)$ is an $x$-string. Then, the set $L=\{w(x;t): t\in [n]\} \subseteq S $ forms a combinatorial line.
%
%Therefore, the set $S$ contains a combinatorial line.
%\end{proof}
...
%  \Jnote{s/at most/exactly}
  one element equals to $1.$ But, for a fixed $t$ there is at least one special $i$ for which $q_i^t=1$. 
% In other words, in a line of the $k \times n$ matrix there is at least one one, that is for $e$ fixed the cardinality of the set $\{i \in [n]: q_i^e=1\}$ is at least one.
%  \Jnote{No, this is false.}
 ...
 % \Jnote{$ = ...?$}
  ...
  %\Jnote{Last condition you have to check, it is not always true.}
  Let us compute the value of $G_S^n$. The element 
 % \Jnote{s/string/element}
