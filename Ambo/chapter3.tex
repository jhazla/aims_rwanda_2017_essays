...
\Jnote{Don't cite here, this is a common definition.}
...
\item  The graph $G=(U;E)=(X,Y;E)$ is \emph{bipartite}, where the vertex set $U=X \cup Y$ is partionned
  \Jnote{s/partionned/partitioned}
...
\Jnote{NO, this is wrong! For $r$ rounds provers and verifier exchange
  $r$ messages. Please delete this.}
...
\Jnote{Remove et al. from citations.}
...
= & \val (G)^n.
\end{align*}
\Jnote{Here the equation becomes true, but the reason for that is
  that $f_i, h_i$ depend only on $x_i, y_i$. Otherwise it is false.} 
...
\Jnote{Make a section for this example.}
...
\end{itemize}
\Jnote{No, the verifier checks rules for $G$ as before. These conditions
  are just for the strategy below.}
...
\Jnote{Typo in the first row of first table. What is
``Projection on expander games''?}
