...
%\Jnote{According to your previous discussion $\mu$ should also be part of the   definition.}
...
When $\val (G)=1$, the game $G$ is called \textit{trivial}. In mostly of cases, we will consider a \textit{non-trivial} game , that is a prover game with $\val (G) \neq 1.$
...
% \Jnote{Be consistent with probability symbol: either $\mathrm{P}$ or $\Pr$    everywhere.}
...
Let $G_{XY}=(X,Y,E)$ a bipartite graph. The expander graph $G_{XY}$ is based on the notions of  singular values (absolute values of the eigenvalues) of the normalized adjacency matrix $M=M(G_{XY})$ of $G_{XY}$, that is where each entry of $M$ is divided by $\sqrt{d_X.d_Y}.$ The singular-value decomposition theorem states that for an $|X|$-by-$|Y|$
 %\Jnote{Proper way to write this in Latex: $|X|$-by-$|Y|$   (the dashes outside math mode).}
...
%\Jnote{Consider deleting this table. In my opinion it is better   to explain just one type of game than to write a long list of names   without explanations.}
...
%\Jnote{Be careful, this equation is true only for your special strategies,   not in general.}
...
%\Jnote{No, $f_i$ is function of $\bar{x}$ and $f$ function of $x_i$.  This is important!}
