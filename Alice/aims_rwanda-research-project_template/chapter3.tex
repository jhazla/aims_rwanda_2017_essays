...
\Jnote{The section title is not relevant to the contents.}
...
$ txt $
\Jnote{Don't use math mode for this. You can use verbatim:
\textbackslash verb+txt+}
files which is considered as dataset. For analysing the data, we used  python programming language.
\Jnote{Capitalize Python.}
...
\item clustering is a technique for analysing data by identifying hidden groups in a data set. The hidden groups helps the machine to  classify them data into small groups called "cluster" based on similarities or relationship found in data \citep{dy2004feature}.
  \Jnote{Some typos here, please correct (also capital letter in the beginning).}
\end{itemize}
...
\Jnote{I don't understand this sentence.}
...
\Jnote{Start with saying what REs are for (to search for patterns
  in strings)}.
...
\Jnote{ASCII and Unicode (note capitalization) are different things
  and you are confusing them here.}
...
\end{itemize}
\Jnote{What are those points for? What is their relation to the rest
  of the paragraph?}
\newpage
\begin{figure}[hbtp]
\caption{ASCII TABLE}
...
\Jnote{Acknowledge the source of the figure.}
...
\Jnote{Please fix grammar above.}
...
eras ,
\Jnote{Delete this beginning and just write ``Some documents can have...''}
...
\Jnote{Do not use ``you'', write in passive voice. Also, what do you
  mean ``to install you must understand how they work and the content type''?
...
\item \textbf{os} : This module is  known as miscellaneous operating system interfaces. os represents the functionality of operating system with independent functions such as os.path.isfile(), os.path.exist, os.path.isdir, etc.  
  ...
  \Jnote{s/andependant platform/platform-indpendent programs}
  The programmes
  \Jnote{s/programmes/programs}
...
\item \textbf{pandas} is an open source with high performance structure within various build in functions.
  ...
  \Jnote{I could not understand this description.}
...
\item  \textbf{defaultdictionary} has basic content of difference between verbs, nouns, adjectives, adverbs etc. It uses tagger to assign each word composes the sentence a corresponding POS as explained in Chapter \ref{tree} classes of words are inverted by NLP, it refers to categories of words in dictionary.
  \Jnote{What do you mean? Defaultdict in Python is just a dictionary
    with default value for missing key instead of key error.
    It has nothing to do with parsing.}
\item \textbf{Python String} module which returns a string with trailing text removed.
  \Jnote{The module does not return anything, its methods do. Their
  names are ``rstrip'' and ``lstrip''.}
...
\end{itemize}
\Jnote{In my opinion the whole section above needs rewriting.}
...
\Jnote{From the figure it looks these are not sentences, just lines.}
\begin{figure}[hbtp]
...
\Jnote{Make capitalization consistent across the thesis:
...
\Jnote{Singular or plural? Be consistent.}
\newpage
...
\end{figure}
\Jnote{Are you sure this figure is correct? It seems there are lots
  of obvious locations classified as ``other''.}
...
\Jnote{Please work more on this chapter. There are many language
  mistakes (I did not point out all of them). Also please stop
  using newpage, Latex will put the tables reasonably on its own.}
