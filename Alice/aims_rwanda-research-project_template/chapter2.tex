...
\Jnote{The text above should be general introduction to this chapter, but you
  write about very specific problems (punctuation, capitalization) which
  are not general. You should put it into an appropriate section.}
...
\Jnote{Full stop after last sentence.}

There are two mains steps which can
\Jnote{s/can/are}
...
\end{itemize}
\Jnote{Last sentence should be after itemize (and in a new paragraph).}
\begin{itemize}
...
\Jnote{Split this sentence into two. Pay attention to grammar.}
\begin{figure}[hbtp]
\caption{General Extraction Process }
\centering
\includegraphics[scale=0.7]{images/pos.png}
\end{figure}
\Jnote{Where is the figure taken from? Please cite properly.}
\Jnote{Please provide citation for this section.}
\section{ Named Entity Recognation and Classification  NERC}
\Jnote{Put ``NERC'' in parentheses in section name.}
...
\end{enumerate}
\Jnote{Spaces after commas above.}


The entities from different reports.
\Jnote{This is not a sentence (no verb).}
...
\Jnote{No space before comma.}
etc.

\section{Hidden Markov Model}
\Jnote{Please say in the beginning what is this model used for
  (in the entity extraction context).}
...
\Jnote{In prose there is always space before parenthesis. So, above
  it should be ``words (SW)''. Only for mathematical functions, like f(x)
  it is the opposite.}
\begin{align}
...
\Jnote{Do not capitalize ``sequence'' or ``tagged''.}
...
\Jnote{s/on another hand/on the other hand}
...
\Jnote{What do you mean by ``masters thesis''?}
We can be ignored P(SW)
\Jnote{We can be ignored/We can ignore}
...
\Jnote{Please say that this estimation is due to assumption that the
  probabilities of tags are independent from each other.}
\begin{align}
...
\Jnote{Why do you enforce new page here? Please don't do that.}
\newpage
\begin{figure}
...
\end{figure}
\Jnote{Fix this figure. You can put two parts on top of each other.}
...
\Jnote{Merge this section with the previous one.}
...
\Jnote{s/than/rather than}
In addition the decision tree has a detailed boolean-like  model which is more popular to user.
\Jnote{Replace ``popular'' with another word (I'm not sure what you mean.
  ``Accessible''?)}

\section{Some Terminologies}
\Jnote{This is not a good title. ``Some Definitions'' would be better.
  Even better if it said what the definitions are related to
  (for example, ``NLP Definitions''}.

\Jnote{I did not understand much from this section. What is IE?
  Definition of hand-written rule does not mention what it is.
  Also other definitions are not clear.
...
\Jnote{Please don't use newpage, here or elsewhere.}
\newpage
{\centering{$\theta$}
...
\Jnote{I did not understand what this algorithm is for and how it
  works. I don't see how the picture is relevant to the algorithm.
  Please provide the source for the picture.}
