...
\section{Results}
 Figure \ref{pics:res}, illustrates the typical spread of infection for each of the five scenarios listed above. One can observe from figure \ref{pics:res} in the first raw, that when $\mu p_2 =0$  is there are very few infections and they occur in clumps.

For $p_1 =0$ we see only isolated infections with no spatial  correlation. As both $p_2$ increase the infection becomes explosive. In raw five we investigate how the infectious period affects the spread of the infection. Since the infectious period is given with large confidence interval we set it to be 50 days instead of 17.4 and investigate its effect. I can be seen that the number of infections in raw five on day 80 is the largest in all our cases. The longer the infectious period the faster the disease is spread.
