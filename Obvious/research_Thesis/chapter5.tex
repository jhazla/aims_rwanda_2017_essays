...
\section{Results}
From our simulated model for the transmission dynamics of Zika virus the following results were obtained. In case 1 where $p_2 =0$, the disease spread in isolated cases. In the first row of figure  \ref{a} it can be seen that at different time interval the number of infected is small and  the infections are in cramps.  The number of individuals infected at each time is stochastic and not too many people will be infected as the disease progresses.  When $p_2 = 0$, the populations is just a regular lattice. The disease does not spread rapidly when there are only local infections. The growth rate of infections in this case is $1.035$.
