...
\Jnote{What is $\mathbb{XT}$? Do you mean Cartesian product? If yes,
  indicate it explicitly with \textbackslash times.
Also, it would be good to add a citation for this.}
...
\Jnote{s/intermediately/immediately}
...
\Jnote{Please use consistent notation. $S(t)$ or $S_{(t)}$.}
...
\Jnote{I don't get this definition. Since $E \subseteq V \times V$, what is
  $f$ for?}
...
\Jnote{Please fix the preceding sentence.}
This can be summed up as the average distance taken over all, pairs of individuals , which gives the idea of the typical distance between nodes in a network or the diameter of the graph which is the largest distance taken over all pairs.
 ...
 \item[i] \textit{\textbf{Degree distribution}}
   ...
   \Jnote{In this context use \textbackslash langle and \textbackslash rangle instead of
     $<$ and $>$.}
 ...
 \Jnote{Mixing formulas and text is bad style. So s/$t_i=$ e number
   of/$t_i$ is a number of}
...
\end{itemize}
 \Jnote{Please give intuitions behind those coefficients as in Phil's lecture.}
 \subsection{Random Graphs} A random graph can be defined as , given a $N$ number of vertices edges between them are drown such that between any pair $i,j$ there is an edge with uncorrelated probability $p$ \citep{newman2002random}.
 \Jnote{No, a random graph can be sampled from any probability distribution.
   What you describe is usually called ``$G(n,p)$ random graph''.}
...
\Jnote{What is small world network property?}
it has an average degree $z= pN$.
\Jnote{Approximately $pN$ (actually $z=p(N-1)$). Also this is the
  \emph{expectation} of
  average degree.}
Random networks have a a low clustering coefficient $ c = \frac{z}{N}$ \citep{newman2003structure}.
\Jnote{Again, this is just approximation of $c$.}
...
\Jnote{Fix the figure: The upper labels are too small and the numbers
  badly formatted. Also, the labels of vertices are too small to read,
  just delete them.}
...
\Jnote{Define what a lattice is or at least draw an example.}
...
\Jnote{Give more precise definition and/or add an example.}
