...
\Jnote{Why is ``Flaviridae'' capitalized and ``Flavivirus'' not? Why is ``virus''
  capitalized in ``Dengue Virus'' but not in ``West Nile virus''? Capitalization
  needs to be applied consistently across the thesis.}

\Jnote{``first case was first isolated''?}

\Jnote{s/diagonised/diagnosed. s/parts o the world/parts of the world.
  Please spellcheck what you wrote
...
\Jnote{In the caption: s/counties/countries}


Zika Virus is mainly spread by Aedes mosquitoes, when the Mosquito bites an infected person it carries the pathogen and when It bites an infected person it also it leaves the pathogen in them. \Jnote{s/Aedes mosquitoes/mosquitoes of genus Aedes. Why is ``Mosquito'' capitalized? s/it also it leaves/it also leaves}
\Jnote{``when it bites an infected person it leaves pathogen'' What are you
  saying here? The person is already infected.}
...
\Jnote{s/ways of by which/ways by which} \Jnote{Capitalization. Just remove
  all capital letters.}

\cite{musso2015}, shows that some of the symptoms are papular rash,fever,arthritis or arthralgia.
\Jnote{Always include a space after comma.} \Jnote{\textbackslash citep is more
  appropriate here.}
In addition ,
\Jnote{Never include a space before comma.}
...
\Jnote{``state adds''? ``during usually''?}
\Jnote{Again, ``X et al. state Y'' is not usual way of stating common facts.
  Better say ``Y'' and use \textbackslash citep. For the sentence above:
  ``Children of mothers infected with Zika during pregnancy
  usually exhibit birth defects
  \textbackslash citep \{...\}''}
\Jnote{Is it really ``usually''?}
...
\Jnote{Sentence above is not grammatical.}
\Jnote{s/replant/repellent}
...
\Jnote{``Drugs for the systems of Zika'' I don't understand what it is.}
\Jnote{s/luck/lack}

\Jnote{The frequency of your language mistakes is not acceptable.
  You have to read through what you wrote, possibly several times,
  before handing it in. You have to read every sentence, correcting
  typos, making sure it is grammatical and meaningful and that
  the meaning is the intended one.}

\Jnote{From now on I will not be marking small mistakes, it is up to you
...
\Jnote{Maybe indicate what is the date to which your figure refers.}
...
Mathematical models for disease transmission have been used to link biological processes of disease transmission and emergent of dynamics of infections at the population level.
\Jnote{s/emergent/emergence}
Researchers try to understand the environmental, biological  and behavioural infectiousness of a disease .
  ...
  \Jnote{I don't understand how your last sentence connects to the rest
    of this paragraph.}
...
\Jnote{The last few paragraphs are disorganized: you should describe historical
...
\Jnote{it's = it is. The possesive is ``its'' (like ``his'', ``her'', etc.).}
...
\Jnote{I looked into this Ming paper and the stochastic model it proposes is
  different from what you describe. Please make your text and citation
  consistent.}
...
\Jnote{You cover a lot of ground in this chapter and it feels disorganized.
  Consider introducing a few sections, topics could be
  e.g., epidemiology, Zika virus, modeling with DEs, modeling with graphs}.
