...
\Jnote{Better say that the name ``Monte Carlo method'' was first used. Your
...
\Jnote{Make clear that you want to sample according to $p(x)$ and that
you assume you can make uniform samples.}
...
\Jnote{Last two sentences are wrong, please correct.}
This causes the inverse transform method to become less efficient \citep{Devroye:1986:SNR:318242.318443}.

\Jnote{How about adding an example, e.g., normal distribution?}

\section{The Accept-reject method}
\Jnote{Be consistent capitalizing section titles.}
...
\Jnote{Make clear what you want to sample and what you assume you can sample.}
...
\Jnote{Make clear you assume $x$ has bounded range. Define $p_{max}$ correctly
  and keep the notation consistent.}
...
\State Begin again
\end{algorithmic}
\Jnote{I think it's better if you use loops instead of ``begin again''.
  Make clear that ``accept'' means ``return $x$ as our sample''
  and ``reject'' means ``do nothing''.}
...
\Jnote{Make this more explicit in a formula.}
...
\Jnote{s/add $x$ to our distribution/return $x$ as a sample from our
  distribution}
...
\Jnote{In the figure: What is $\epsilon$?}
...
\Jnote{Pseudorandomness has to do with computers, not Monte Carlo
  specifically. It is possible to run MC with ``real'' randomness.}
...
\Jnote{Decreasing how fast?}

\Jnote{Why not have a separate section on $\pi$? You used to write more about
  this.}

\Jnote{Why not compare accept-reject and inverse transform? Without such
  discussion what is the point of presenting two methods to do the same
  thing?}
