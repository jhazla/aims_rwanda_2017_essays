...
\Jnote{No need to put colon in math mode. Also: This colon (and the
preceding clause) is not grammatical.}
...
$(forces)$
\Jnote{Never use math mode to change font of text. Probably you want something
  like \textbackslash emph\{\} here.}
to construct matter as we see it.
\Jnote{There are a couple of grammatical mistakes in this sentence. Also,
it is very long.}
...
\Jnote{Is colour charge the same as electric charge? It is not clear to me
  from this sentence.}
...
\Jnote{What about when the rubber band ``breaks''? Can the macroscopic
  distance be achieved then?}
...
\Jnote{No manual lists, use itemize.} 
...
\Jnote{Please refer to the table from the text.}
...
\Jnote{You can see you have a lot of language problems. You have to read
  what you wrote, possibly several times. You have to read every sentence,
  correcting typos, making sure it is grammatical and meaningful and that
  the meaning is the intended one.
  Pay special attention to -s endings in 3rd person.}

\Jnote{From now on I will not be marking minor language mistakes, you
  have to find and correct them yourself.}

\section{Large Hadron Collider}

Hadron colliders are devices made to explore the world of particle physics, they work as theories testers and also as a discovery machines, an example of these hadron colliders is the LHC in CERN. In the Large hadron collider
...
\Jnote{``higher order real emission corrections'', ``evolution variable
  representing the angular ordering'', don't use lots of complicated words
...
\Jnote{I wouldn't call it ``example'' of what you described before.
  It is a related thing, but an example should give specific
  $f$ and $p$.}

\Jnote{The pi figure: 1) Caption would be nice. 2) Why uncertainty does
  not go to 0? Can you explain this?}

\begin{figure}
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[scale=.4]{images/pi_value.png} 
        \caption{pi value}
        \label{fig:gull}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[scale=0.4]{images/uncertainity_pi.png}
...
x = p^-1(y)
\end{equation}
\Jnote{You need to put braces for superscripts, like this: p\^{}\{-1\}}
...
\Jnote{``Inversely distributed''? What does that mean?}
where the small angles are desired. The histograms in figure 2 demonstrate this and the error in the calculation.
\Jnote{What do they demonstrate? What exactly is the distribution?}

\Jnote{Why does this work? Consider writing formula for cdf to give the reader
  an idea. Also, in this method cdf is used, not pdf as you write.}
...
\Jnote{Don't use math mode to write Python. Use capital letter.
  For module, variable and file names you can use verbatim command like this:
...
\Jnote{Please use mathematical notation instead of Python code.} 
The underlying implementation in C is both fast and threadsafe.
\Jnote{What does ``threadsafe'' mean? Explain or drop this.}
The Mersenne Twister is one of the most extensively tested random number generators in existence.

\Jnote{Where does this section come from? Citation please.}
