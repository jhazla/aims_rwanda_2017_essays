...
\Jnote{General comment:
  It feels to me you deleted a lot of text which either was good or
  needed changes, not deleting.}

\Jnote{I understand that Sam brought up the issue of plagiarism, but please do not
  react in this way. You are allowed to write things that you learnt,
  both from literature and Sam's tutorial. The only thing that is (rightly)
  required is that you put them in your own words.}

\Jnote{At the risk of repeating myself for 100th time: If you first read
  and then write ``closed-book'' (without looking at what you read while
  writing), you will be safe from plagiarism accusations.}
...
\Jnote{You could say a bit about previous colliders and their achievements.
  You could mention where CERN is.}

\Jnote{You could mention how long ago Higgs boson was proposed and how big
  deal it was to confirm it (it was the last particle in SM to be verified,
  no?), citing appropriate papers.}

Hadron colliders are devices made to explore the world of particle physics, they work as theories testers
\Jnote{s/theories/theory}
...
\Jnote{Do you mean sth like ``SM prediction'' here?}
that the masses of the elementary particles are generated through the Higgs mechanism \citep{particle}.
\Jnote{Rephrase or split into two sentences.}
\Jnote{Be consistent with capitalization of ``standard model''.}
...
Inside the Large hadron Collider, $10^{11}$ protons are being accelerated to a high kinetic energy and then collided up 40 million times per second to provide 14 \si{TeV} proton-proton collision.
\Jnote{s/up/up to. Capitalization. ``Provide'' is not the best word (``create''?).}
...
\Jnote{Maybe explain what is ``collimated''?}



\section{Quarks and Gluons}
\Jnote{Say what are quarks first (you used to do that...).}
...
\Jnote{What are mesons and baryons? You used to explain and I found that helpful.}
...
The other partons (glouns)
\Jnote{s/glouns/gluons}
...
\Jnote{Grammar. ``Similar to quarks, they carry color charge...''}
...
\Jnote{Don't use math mode for emphasis.}
...
%The theory which describes this force is called $Quantum$ $Chromodynamics$ \citep{particle}.
 ...
 \Jnote{Don't define what event is mid-sentence.}
 Monte Carlo event generators provide a good approximation of collisions.\label{sec}
 \Jnote{Since ``direct'' calculations are computationally difficult/expensive,
   right?}
